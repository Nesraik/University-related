...
void PrintAccount(struct node *root){
	if(root!=NULL){
		printf("%s %s %d\n",root->Username,root->Password,root->key);
		PrintAccount(root->left);
		PrintAccount(root->right);
	}
}
...
void AddHashPass(char *Apps,char *Pass,int HashIn,struct node *Current,char *Usernametest){
	
	struct Hash *NewHash = (struct Hash*)malloc(sizeof(struct Hash));
	strcpy(NewHash->Application,Apps);
	strcpy(NewHash->PasswordApplication,Pass);
	strcpy(NewHash->UsernameApp,Usernametest);

	NewHash->next=NULL;
	
	if(Current->HashTable[HashIn]==NULL){
		Current->HashTable[HashIn]=NewHash;
	}else{
		struct Hash *Current2 = Current->HashTable[HashIn];
		while(Current2->next!=NULL){
			Current2=Current2->next;
		}
		Current2->next=NewHash;
	}
}

void AddPassword(struct node *root){
	char Application[30],Passwordtest[30];
	int sum=0,length;
	...
	int length2;
	char Usernametest[30];
	do{
		printf("Username: ");
		fflush(stdin);
		scanf("%s",Usernametest);
		length2 = strlen(Usernametest);
	}while(length2 < 5 || length2 > 30);
	...
	srand(time(NULL));
	for(int i=0;i<29;i++){
		Passwordtest[i] = rand() % (122-97+1)+97;
	}

	int HashKey = sum2 % Size;
	
	AddHashPass(Application,Passwordtest,HashKey,Data,Usernametest);
	printf("<<Password Added>>\n");
	
}

void DisplayPassword(struct node *root){
	...
	struct node *Data = Search(root,sum);
	char Applicationtest[30];
	printf("Enter Application: ");
	fflush(stdin);
	scanf("%[^\n]",Applicationtest);
	getchar();
	
	length = strlen(Applicationtest);
	sum=0;
	for(int i=0;i<length;i++){
		sum+=Applicationtest[i];
	}
	...
	if(Data->HashTable[HashKey]==NULL){
		printf("You havent created a password for this account yet\n");
		return;
	}else{
		
		printf("<<Application: %s>>\n\n",Data->HashTable[HashKey]->Application);
		struct Hash *Current = Data->HashTable[HashKey];
		while(Current!=NULL){
			printf("Username: %s\n",Current->UsernameApp);
			printf("Password: %s\n\n",Current->PasswordApplication);
...
void MainMenu(struct node **root){
	int menu;
	do{
